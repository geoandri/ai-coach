...
function buildFullPlan(
  ...
  const weeks = db.select().from(weeklyBlocks).where(eq(weeklyBlocks.trainingPlanId, plan.id)).all()
    .sort((a, b) => a.weekNumber - b.weekNumber)

  const weekDtos = weeks.map((week) => {
    const workouts = db.select().from(dailyWorkouts).where(eq(dailyWorkouts.weeklyBlockId, week.id)).all()
...
export function getFirstPlanWeek(weekNumber: number): WeeklyBlockDto | null {
  ...
  const week = db.select().from(weeklyBlocks)
...
export function getPlanForAthlete(athleteId: number): TrainingPlanDto | null {
  const plan = db.select().from(trainingPlans).where(eq(trainingPlans.athleteId, athleteId)).get()
  if (!plan) return null
...
export function createPlanForAthlete(
  ...
  const existing = db.select().from(trainingPlans).where(eq(trainingPlans.athleteId, athleteId)).get()
  if (existing) return { error: `Athlete ${athleteId} already has a training plan. Delete it first.`, status: 409 }

  const plan = db.insert(trainingPlans).values({
    athleteId,
    name: req.name,
  ...
  for (const weekReq of req.weeks ?? []) {
    const week = db.insert(weeklyBlocks).values({
...
export function updateWeekForAthlete(
  ...
  const existingWeek = db.select().from(weeklyBlocks)
    .where(and(eq(weeklyBlocks.trainingPlanId, plan.id), eq(weeklyBlocks.weekNumber, weekNumber)))
    .get()
  if (!existingWeek) return { error: `Week ${weekNumber} not found`, status: 404 }

  db.update(weeklyBlocks).set({
  ...
  if (req.workouts) {
    const existingWorkouts = db.select().from(dailyWorkouts)
...
export function deletePlanForAthlete(athleteId: number, planId: number): boolean {
  const plan = db.select().from(trainingPlans)
